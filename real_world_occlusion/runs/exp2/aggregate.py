...
def collect_results(base_dir):
    ...
    for run_dir in run_dirs:
        ...
        json_files = glob.glob(os.path.join(task_path, "*_results.json"))
        if not json_files:
            print(f"Warning: No result JSON files found in {task_path}, skipping.")
            continue
    ...
    for model_name, levels in sorted(model_data.items()):
        ...
        for level in OCCLUSION_LEVELS:
            ...
            if len(values) == 0:
                ...
                mean_acc = round(values[0], 1)
                sd_acc = 0.0
            else:
                mean_acc = round(statistics.mean(values), 2)
                sd_acc = round((statistics.stdev(values) / (len(values) ** 0.5)), 2)  # sample std dev
