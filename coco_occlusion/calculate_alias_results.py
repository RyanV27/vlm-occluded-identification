...
from evaluation_config import ALIASES
...
def calculate_accuracy_occluded_object(model_name, out_path, occ_level):
    results_dir = out_path / model_name           # / "identify_occluded_object"
    results_dir.mkdir(parents=True, exist_ok=True)
    results_file = results_dir / f"{model_name}_results.json"

    # If results file already exists then load previous results
    ...
    #     "medium",
    #     "high"
    # ]   # _occlusion_generation
    if isinstance(occ_level, list):
        occlusion_types = occ_level
    elif isinstance(occ_level, str):
        occlusion_types = [occ_level]

    object_types = set(["occluded_category", "occluder_category", "categories_in_image"])

    # Counter for number of correct identifications of occluded object
    correct = {oc_type: 0 for oc_type in occlusion_types}

    oc_type_overall_results = {}
    for oc_type in occlusion_types:
        oc_type_overall_results[oc_type] = {}
        
        individual_results = {}
        results["individual"] = {}

        generations_file = out_path / model_name / f"{model_name}_{oc_type}_generations.json"
        if not os.path.exists(generations_file):
            print(f"Error: {generations_file} not found.")
            return
        ...
        for key, entry in data.items():
            ...
            occluded_object = entry["occluded_category"]
            ...
            individual_results[key]["occluded_category"] = occluded_object
            individual_results[key]["occluder_category"] = entry["occluder_category"]
            individual_results[key]["categories"] = objects    
            ...
            if obj_matched:
                correct[oc_type] += 1
                individual_results[key][f"is_occluded_object_identified"] = True
            else:
                individual_results[key][f"is_occluded_object_identified"] = False

        # print(f"Individual results:\n{individual_results}")
        results["individual"][oc_type] = individual_results

        oc_type_overall_results[oc_type]["total_scenes"] = total
        oc_type_overall_results[oc_type]["num_correct"] = correct[oc_type]
        oc_type_overall_results[oc_type]["accuracy"] = round(correct[oc_type] / total * 100, 2)
    ...
    for oc_type in occlusion_types:
        print(f"\n{oc_type}:")
        print(f"\tAccuracy = {oc_type_overall_results[oc_type]['accuracy']}")

    # Update and save results
    results["overall"] = oc_type_overall_results
    # results["individual"] = individual_results
    with open(results_file, "w") as f:
        json.dump(results, f, indent=4)
...
def main():
    ...
    calculate_accuracy_occluded_object(args.model_name, Path(args.out_path), args.occ_level)
